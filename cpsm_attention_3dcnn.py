...
def cpsm_train(path, train_patient, test_patient):
    train_files = os.listdir(os.path.join(path, train_patient))
    
    
    X_train, Y_train = get_data(os.path.join(path, train_patient), train_files[0])
    if len(train_files) > 1:
        for file in train_files[1:]:
            Xt, Yt = get_data(os.path.join(path, train_patient), file)
            X_train = np.concatenate((X_train, Xt))
            Y_train = np.concatenate((Y_train, Yt))
    
    for test in test_patient:
        test_files = os.listdir(os.path.join(path, test))
        X_test, Y_test = get_data(os.path.join(path, test), test_files[0])
        if len(test_files) > 1:
            for file in test_files[1:]:
                Xt, Yt = get_data(os.path.join(path, test), file)
    ...
    model = model2(main_input, X_train.shape[1])
    ...
    information = {
        'save': True,
        'save_file_path': '/Gdrive/Colab Notebooks/whhhhh/cpsm/attention/',
        'save_file_name': test_patient,
    }

    CR = ClassificationReportCallback(validation_data=(X_test, Y_test), info=information)

    histroy = model.fit(X_train, Y_train, batch_size=8, epochs=100,
                        validation_split=0.2, verbose=1, callbacks=[CR])

    del model
    K.clear_session()
    
path = '/Gdrive/Data/WHH/30'

patients = ['chb03','chb06','chb10']
for patient in patients:
    
    train_file = patient
    test_file = list(patients)
    test_file.remove(patient) # Inplace, Return X
    
    print(train_file, test_file)
    
    cpsm_train(path, train_file, test_file)